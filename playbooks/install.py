# wget setup_metel.py | python
import os, sys, subprocess, getpass, json, multiprocessing, shutil, platform
from distutils.spawn import find_executable

tmp_zinuit_repo = '/tmp/.zinuit'

def install_zinuit(args):
	check_distribution_compatibility()
	check_brew_installed()
	# pre-requisites for zinuit repo cloning
	...
	extra_vars.update(metel_user=args.user)

	if os.path.exists(tmp_zinuit_repo):
		repo_path = tmp_zinuit_repo

	else:
		repo_path = os.path.join(os.path.expanduser('~'), 'zinuit')
	...
	if args.metel_branch:
		metel_branch = args.metel_branch
	else:
		metel_branch = 'master' if args.production else 'develop'
	extra_vars.update(metel_branch=metel_branch)

	if args.redapple_branch:
		redapple_branch = args.redapple_branch
	else:
		redapple_branch = 'master' if args.production else 'develop'
	extra_vars.update(redapple_branch=redapple_branch)
	
	zinuit_name = 'zemetel' if not args.zinuit_name else args.zinuit_name
	extra_vars.update(zinuit_name=zinuit_name)

	# Will install Redapple production setup by default
...
def check_brew_installed():
	...
	if not brew_exec:
		raise Exception('''
		Please install brew package manager before proceeding with zinuit setup. Please run following
		to install brew package manager on your machine,

		/usr/bin/ruby -e "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/master/install)"
		''')

def clone_zinuit_repo(args):
	'''Clones the zinuit repository in the user folder'''
	if os.path.exists(tmp_zinuit_repo):
		return 0

	elif args.without_zinuit_setup:
		clone_path = os.path.join(os.path.expanduser('~'), 'zinuit')

	else:
		clone_path = tmp_zinuit_repo

	branch = args.zinuit_branch or 'master'
	repo_url = args.repo_url or 'https://github.com/amonak/zinuit'


	success = run_os_command(
		{'git': 'git clone {repo_url} {zinuit_repo} --depth 1 --branch {branch}'.format(
			repo_url=repo_url, zinuit_repo=clone_path, branch=branch)}
	)
...
def get_passwords(args):
	...
	and creates passwords.txt in the zinuit user's home directory
	"""

	ignore_prompt = args.run_travis or args.without_zinuit_setup
...
def run_playbook(playbook_name, sudo=False, extra_vars=None):
	...
	if os.path.exists(tmp_zinuit_repo):
		...
		cwd = os.path.join(os.path.expanduser('~'), 'zinuit')
...
def parse_commandline_args():
	...
	parser = argparse.ArgumentParser(description='Metel Installer')
	...
	args_group.add_argument('--production', dest='production', action='store_true',
		default=False, help='Setup Production environment for zinuit')

	parser.add_argument('--site', dest='site', action='store', default='redweb.local',
		help='Specifiy name for your first Redapple site')
	...
	parser.add_argument('--user', dest='user', help='Install zemetel for this user')

	parser.add_argument('--zinuit-branch', dest='zinuit_branch', help='Clone a particular branch of zinuit repository')

	parser.add_argument('--repo-url', dest='repo_url', help='Clone zinuit from the given url')

	parser.add_argument('--metel-repo-url', dest='metel_repo_url', action='store', default='https://github.com/amonak/metel',
		help='Clone metel from the given url')

	parser.add_argument('--metel-branch', dest='metel_branch', action='store',
		help='Clone a particular branch of metel')
	
	parser.add_argument('--redapple-repo-url', dest='redapple_repo_url', action='store', default='https://github.com/amonak/redapple',
		help='Clone redapple from the given url')
	
	parser.add_argument('--redapple-branch', dest='redapple_branch', action='store',
		help='Clone a particular branch of redapple')

	parser.add_argument('--without-redapple', dest='without_redapple', action='store_true', default=False,
		help='Prevent fetching Redapple')
	...
	parser.add_argument('--without-zinuit-setup', dest='without_zinuit_setup', action='store_true', default=False,
		help=argparse.SUPPRESS)
	
	# whether to overwrite an existing zinuit
	parser.add_argument('--overwrite', dest='overwrite', action='store_true', default=False,
		help='Whether to overwrite an existing zinuit')
	...
	parser.add_argument('--zinuit-name', dest='zinuit_name', help='Create zinuit with specified name. Default name is zemetel')
...
if __name__ == '__main__':
	...
	print('''Metel/Redapple has been successfully installed!''')
