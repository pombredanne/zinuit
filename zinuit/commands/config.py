...
@click.command('update_zinuit_on_update')
@click.argument('state', type=click.Choice(['on', 'off']))
...
def remove_common_config(keys):
	from zinuit.config.common_site_config import get_config, put_config
...
config.add_command(config_update_zinuit_on_update)
