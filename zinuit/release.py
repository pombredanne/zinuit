...
def release(zinuit_path, app, bump_type, from_branch='develop', to_branch='master',
		remote='upstream', owner='metel', repo_name=None, frontport=True):

	confirm_testing()
	config = get_config(zinuit_path)

	if not config.get('release_zinuit'):
		print('zinuit not configured to release')
		sys.exit(1)
	...
	validate(zinuit_path, config)

	bump(zinuit_path, app, bump_type, from_branch=from_branch, to_branch=to_branch, owner=owner,
		repo_name=repo_name, remote=remote, frontport=frontport)

def validate(zinuit_path, config):
...
def confirm_testing():
	...
	print('Manual Testing Checklisk: https://github.com/amonak/zinuit/wiki/Testing-Checklist')
	print('')
	print('')
	click.confirm('Is manual testing done?', abort = True)
	click.confirm('Have you added the change log?', abort = True)

def bump(zinuit_path, app, bump_type, from_branch, to_branch, remote, owner, repo_name=None, frontport=True):
	assert bump_type in ['minor', 'major', 'patch', 'stable', 'prerelease']

	repo_path = os.path.join(zinuit_path, 'apps', app)
	push_branch_for_old_major_version(zinuit_path, bump_type, app, repo_path, from_branch, to_branch, remote, owner)
...
def create_github_release(repo_path, tag_name, message, remote='upstream', owner='metel', repo_name=None,
...
def push_branch_for_old_major_version(zinuit_path, bump_type, app, repo_path, from_branch, to_branch, remote, owner):
	if bump_type != 'major':
		return
