import zinuit, os, click, errno
from zinuit.utils import exec_cmd, CommandFailedError, update_common_site_config
from zinuit.config.site_config import update_site_config, remove_domain, get_domains
from zinuit.config.nginx import make_nginx_conf
from zinuit.config.production_setup import service
...
def setup_letsencrypt(site, custom_domain, zinuit_path, interactive):

	site_path = os.path.join(zinuit_path, "sites", site, "site_config.json")
	...
	run_certbot_and_setup_ssl(site, custom_domain, zinuit_path, interactive)
	setup_crontab()


def create_config(site, custom_domain):
	config = zinuit.env.get_template('letsencrypt.cfg').render(domain=custom_domain or site)
...
def run_certbot_and_setup_ssl(site, custom_domain, zinuit_path, interactive=True):
	service('nginx', 'stop')
	get_certbot()
