...
def generate_config(zinuit_path):
	...
	# make pids folder
	pid_path = os.path.join(zinuit_path, "config", "pids")
	if not os.path.exists(pid_path):
		os.makedirs(pid_path)

def write_redis_config(template_name, context, zinuit_path):
	template = zinuit.env.get_template(template_name)

	if "pid_path" not in context:
		context["pid_path"] = os.path.abspath(os.path.join(zinuit_path, "config", "pids"))

	with open(os.path.join(zinuit_path, 'config', template_name), 'w') as f:
